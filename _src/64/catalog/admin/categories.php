...
// removing categories can be a lengthy process
      ...
      case 'update_product':
        if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
          ...
          $products_expires_date = tep_db_prepare_input($HTTP_POST_VARS['products_expires_date']);

          $products_expires_date = (date('Y-m-d') < $products_expires_date) ? $products_expires_date : 'null';

          $sql_data_array = array('products_quantity' => tep_db_prepare_input($HTTP_POST_VARS['products_quantity']),
                                  'products_model' => tep_db_prepare_input($HTTP_POST_VARS['products_model']),
                                  'products_price' => tep_db_prepare_input($HTTP_POST_VARS['products_price']),
                                  'products_date_available' => $products_date_available,
                                  'products_expires_date' => $products_expires_date,
        ...
        break;
      case 'reset_product':
			    tep_db_query("update " . TABLE_PRODUCTS . " set products_date_added = now(), products_last_modified = now() where products_id = '" . $HTTP_GET_VARS['pID'] . "'");
      ...
      case 'copy_to_confirm':
        if (isset($HTTP_POST_VARS['products_id']) && isset($HTTP_POST_VARS['categories_id'])) {
          ...
          if ($HTTP_POST_VARS['copy_as'] == 'link') {
            ...
            $product_query = tep_db_query("select products_quantity, products_model, products_image, products_price, products_date_available, products_expires_date, products_date_status_change, products_weight, products_tax_class_id, manufacturers_id from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
            $product = tep_db_fetch_array($product_query);

            tep_db_query("insert into " . TABLE_PRODUCTS . " (products_quantity, products_model,products_image, products_price, products_date_added, products_date_available, products_expires_date, products_date_status_change, products_weight, products_status, products_tax_class_id, manufacturers_id) values ('" . tep_db_input($product['products_quantity']) . "', '" . tep_db_input($product['products_model']) . "', '" . tep_db_input($product['products_image']) . "', '" . tep_db_input($product['products_price']) . "',  now(), " . (empty($product['products_date_available']) ? "null" : "'" . tep_db_input($product['products_date_available']) . "'") . ", " . (empty($product['products_expires_date']) ? "null" : "'" . tep_db_input($product['products_expires_date']) . "'") . ", " . (empty($product['products_date_status_change']) ? "null" : "'" . tep_db_input($product['products_date_status_change']) . "'") . ", '" . tep_db_input($product['products_weight']) . "', '0', '" . (int)$product['products_tax_class_id'] . "', '" . (int)$product['manufacturers_id'] . "')");
...
<?php
  if ($action == 'new_product') {
    ...
    if (isset($HTTP_GET_VARS['pID']) && empty($HTTP_POST_VARS)) {
      $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, date_format(p.products_expires_date, '%Y-%m-%d') as products_expires_date, p.products_status, p.products_tax_class_id, p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
      $product = tep_db_fetch_array($product_query);
...
<script language="javascript"><!--
  ...
  var dateExpire = new ctlSpiffyCalendarBox("dateExpire", "new_product", "products_expires_date","btnDate1","<?php echo $pInfo->products_expires_date; ?>",scBTNMODE_CUSTOMBLUE);
//--></script>
<script language="javascript"><!--
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            <td class="main"><?php echo TEXT_EXPIRES_DATE; ?><br><small>(YYYY-MM-DD)</small></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;'; ?><script language="javascript">dateExpire.writeControl(); dateExpire.dateFormat="yyyy-MM-dd";</script>&nbsp;<?php echo TEXT_TIP; ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
...
<?php
    ...
    $products_count = 0;
    if (isset($HTTP_GET_VARS['search'])) {
      $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_quantity, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_expires_date, p.products_date_status_change, p.products_status, p2c.categories_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and pd.products_name like '%" . tep_db_input($search) . "%' order by pd.products_name");
    } else {
      $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_quantity, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_expires_date, p.products_date_status_change, p.products_status from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$current_category_id . "' order by pd.products_name");
    }
      ...
      default:
        if ($rows > 0) {
          if (isset($cInfo) && is_object($cInfo)) { // category info box contents
            ...
            if (date('Y-m-d') < $pInfo->products_expires_date) $contents[] = array('text' => TEXT_INFO_EXPIRES_DATE . ' ' . tep_date_short($pInfo->products_expires_date));
            if (tep_not_null($pInfo->products_date_status_change)) $contents[] = array('text' => TEXT_INFO_STATUS_CHANGE . ' ' . tep_date_short($pInfo->products_date_status_change));
