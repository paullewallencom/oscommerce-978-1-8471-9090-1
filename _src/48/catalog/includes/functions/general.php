...
// nl2br() prior PHP 4.2.0 did not convert linefeeds on all OSs (it only converted \n)
  ...
  function tep_get_manufacturers_name($products_id) {
    ...
    $manufacturers_query = tep_db_query("select m.manufacturers_name from " . TABLE_MANUFACTURERS . " m, " . TABLE_PRODUCTS . " p where p.manufacturers_id = m.manufacturers_id and p.products_id = '" . $products_id . "'");
...
// Construct a category path to the product in names
  function tep_get_product_path_names($products_id) {
    ...
    if (tep_db_num_rows($category_query)) {
      ...
      $cat_namePath = tep_get_category_name(implode('_', $categories));

      if (tep_not_null($cat_namePath)) $cat_namePath .= '->';
      $cat_namePath .= tep_get_category_name($category['categories_id']);
    }
