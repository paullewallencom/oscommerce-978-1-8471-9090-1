      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_FLAT1_ZONE > 0) ) {
        ...
        if ($check_flag == false) {
          $this->enabled = false;
			...
			if (is_object($order)) {
			// hide the module if the order contains more than one item, or is not from the specified category
				if ($this->enabled == true) {
					...
					if ($cart->count_contents() == 1) {
						...
						//check if the item belongs to the cat specified for flat fee shipping						
						$item_path = tep_get_product_path(tep_get_prid($products_id));
...
// class methods
    ...
    function install() {
			tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Flat Fee for single item order in a specified category', 'MODULE_SHIPPING_FLAT1_STATUS', 'True', 'Do you want to offer Flat Fee for single item order in a specified category?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())"); 
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Category', 'MODULE_SHIPPING_FLAT1_CAT_LIMIT', '0', 'This module is limited to products in this category.', '6', '0', 'tep_get_categories_title', 'tep_cfg_pull_down_topcategories(', now())");
    ...
    function keys() {
      return array('MODULE_SHIPPING_FLAT1_STATUS', 'MODULE_SHIPPING_FLAT1_COST', 'MODULE_SHIPPING_FLAT1_TAX_CLASS', 'MODULE_SHIPPING_FLAT1_ZONE', 'MODULE_SHIPPING_FLAT1_CAT_LIMIT', 'MODULE_SHIPPING_FLAT1_SORT_ORDER');
    }
  }
?>
