...
// make sure no duplicate category IDs exist which could lock the server in a loop
	...
	function tep_cfg_pull_down_topcategories($categories_id, $key = '') {
	 global $languages_id;

	 $name = (($key) ? 'configuration[' . $key . ']' : 'configuration_value');

	 $cat_query = tep_db_query("select c.categories_id, cd.categories_name from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and c.parent_id = '0' and cd.language_id = '" . (int)$languages_id . "' order by categories_name");
	 while ($cat = tep_db_fetch_array($cat_query)) {
		$cat_array[] = array('id' => $cat['categories_id'],
								 'text' => $cat['categories_name']);
	 }

	 return tep_draw_pull_down_menu($name, $cat_array, $categories_id);
	}

	function tep_get_categories_title($categories_id, $language_id = '') {
	 global $languages_id;

	 if (!is_numeric($language_id)) $language_id = $languages_id;

	 $cat_query = tep_db_query("select categories_name from " . TABLE_CATEGORIES_DESCRIPTION . " where categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$language_id . "'");
