...
// - The expiry date is not reached
      if ( ($downloads['download_count'] > 0) && (file_exists(DIR_FS_DOWNLOAD . $downloads['orders_products_filename'])) && ( ($downloads['download_maxdays'] == 0) || ($download_timestamp > time())) ) {
        echo '            <td class="main"><a target="_blank" onClick="window.location.reload();" href="' . tep_href_link(FILENAME_DOWNLOAD, 'order=' . $last_order . '&id=' . $downloads['orders_products_download_id']) . '">' . $downloads['products_name'] . ' - ' . $downloads['orders_products_filename'] . '</a></td>' . "\n";
      } else {
        echo '            <td class="main">' . $downloads['products_name'] . ' - ' . $downloads['orders_products_filename'] . '</td>' . "\n";
      }
?>
<!-- right box -->
<?php
      echo '            <td class="main">' . TABLE_HEADING_DOWNLOAD_DATE . tep_date_short($download_expiry) . '</td>' . "\n" .
